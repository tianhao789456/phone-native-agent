...
object NativeAgentProfile {
    ...
    val systemPrompt: String =
        "Prompt version: $SYSTEM_PROMPT_VERSION. " +
            ...
            "For repeated tasks, phone-control tasks, desktop/MCP tasks, terminal tasks, SSH/file-transfer tasks, and user-specific questions, inspect reusable skills first with skill_list; use skill_read for exactly one matching skill, and skill_run only when execution is appropriate. Skill listings are compact and cache-friendly; procedures and plugin workflows are details loaded on demand. Then check experience and memory when useful. After tool-based tasks, record durable user facts and reusable success/failure lessons with memory or experience tools when useful. Do not store secrets verbatim. " +
